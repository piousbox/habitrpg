...
api.updatePassword = {
  method: 'POST',
  middleware: [authWithHeaders(), cron],
  ...
  async handler (req, res) {
    ...
    req.checkBody({
      password: {
        netEmpty: {errorMessage: res.t('missingNewPassword')},
      },
      newPassword: {
    ...
    user.auth.local.hashed_password = passwordUtils.encrypt(req.body.newPassword, user.auth.local.salt);
    user.save();
...
api.updateUsername = {
  ...
  async handler (req, res) {
    ...
    if (candidateUser) throw new NotAuthorized(res.t('usernameTaken'));
